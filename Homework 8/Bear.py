...
#Defining print statement output
    def __str__(self) : 
        ...
        return string
#Creating/Defining movement options for bears based on current trajectory
    def walk(self) : 
        for i in range(0,len(self.active)) : 
            ...
            if self.active[i][2] == "N" :
                ...
                self.active[i][0] = self.active[i][0] - 1 
                self.active[i][1] = self.active[i][1] + 1
            if self.active[i][1] == -1 or self.active[i][0] == -1 or self.active[i][1] > len(self.active) or self.active[i][0] > len(self.active[i]): 
                self.active.remove(self.active[i])
#Establishing a function responsible for bears consumption of berries 
    def consume(self,field,tourists) : 
        ...
        for i in range(len(self.active)) : 
            if self.active[i][3] < 30 and ([self.active[i][0],self.active[i][1]] not in self.tourists): 
                array_field[self.active[i][0]-1][self.active[i][1]-1] = 0 
        return array_field
